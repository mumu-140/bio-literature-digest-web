...
- `bio-literature-digest-web/bio-literature-config/` is the live local-only config and runtime data root for the web project.
...
- `bio-literature-config/web/access-traces`: append-only per-user entry traces.
- `bio-literature-config/web/review-tables`: delayed review export output, including weighted aggregate sheets.
