...
class AuthAndFavoritesTest(unittest.TestCase):
    ...
    def _seed_imported_paper(self) -> int:
        with database.SessionLocal() as db:
            ...
            paper = ImportedLiteratureItem(
                literature_item_key="doi:10.1000/shared",
                doi="10.1000/shared",
    ...
    def test_favorite_reuses_local_imported_metadata(self) -> None:
        with TestClient(self.app_factory()) as client:
            ...
            with database.SessionLocal() as db:
                ...
                paper.title_en = "Updated local imported title"
                db.commit()

            favorites = client.get("/api/favorites")
            self.assertEqual(favorites.status_code, 200)
            self.assertEqual(len(favorites.json()), 1)
            self.assertEqual(favorites.json()[0]["title_en"], "Updated local imported title")
            self.assertEqual(favorites.json()[0]["canonical_key"], "doi:10.1000/shared")
