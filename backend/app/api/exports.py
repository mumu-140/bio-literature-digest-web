...
from ..models import User, UserExportJobV2
...
def serialize_job(job: UserExportJobV2) -> ExportJobRead:
...
def get_export_job(
    ...
    job = db.get(UserExportJobV2, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Export job not found")
    if current_user.role != "admin" and job.requested_by_user_id not in {None, current_user.id} and job.requested_by_key != current_user.email.lower():
